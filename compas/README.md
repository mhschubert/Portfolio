...
The analysis shown is the more accessible version shown in the working paper "ode is Law: How COMPAS Affects the Way the Judiciary Handles the Risk of Recidivism" (Chapter 4 of the PhD dissertation).
...
├───code
    ...
    ├───notebooks (notebook showing the analysis and story)
    ├───plot_utils
    └───ml_utils
└───data
    ├───preprocessed (preprocessed features and y in csv format)
    ├───train_val_test (split dataset
    └───Raw_data

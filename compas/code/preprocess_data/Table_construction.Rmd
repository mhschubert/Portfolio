...
```{r, warning=FALSE, message=FALSE}
#decide whether to calculate violence feaures or non-violence ones
violence = FALSE
print(paste('We are calcualting the information with arrests?:', violence, sep=' '))
```
...
# Data we have pulled
 f_our_data = "../../data/Raw_data/probation/"
...
compas_df_wide = compas_df2 %>%
  select(person_id, screening_date, type_of_assessment, raw_score, decile_score,score_text, marital_status) %>%
  gather("key","value",raw_score,decile_score, score_text) %>%
...
features = pmap_dfr(data_before, .f=compute_features, violence=violence)
...
```
We added this here so that we can calculate whether the person was imprisoned as a consequence of their arrest. To that end we look whether there is a charge against them after their screening but before the next screening and then whether an imprisonment follows that. Naturally, this method is not foolproof. It may be that they are released on bail and commit an offense. A new sceening date is logged. However, the charge date is then after the second screening date although it is connected to the first offense. At the moment, we have no way around this problem. We look for a charge relating to an offense date.


```{r}
dyn_charge = key_events %>%
  left_join(charge_df, by="person_id") %>% 
  filter(as.Date(offense_date) == as.Date(before_cutoff_date))  %>%
  #filter(as.period(interval(offense_date, before_cutoff_date), 'days')<=10) %>% 
  select(-first_offense_date, -current_offense_date, -offenses_within_30, -before_cutoff_date) %>%
  group_by(person_id, screening_date) %>%
  nest(.key="charge")

#now we subset to only have the id/screening date combinations fulfilling the above
...
```{r}
### Compute features (e.g., number of priors) for each person_id/screening_date combination.
features_before_on = pmap_dfr(data_before_on, .f=compute_features, violence = violence)
...
dyn_jail_after = key_events %>%
  left_join(jailhistory_df, by="person_id") %>% 
  filter(out_custody >= before_cutoff_date) %>% 
  select(-first_offense_date, -current_offense_date, -before_cutoff_date) %>% 
  group_by(person_id, screening_date) %>%
  nest(.key="jail") 
...
```{r}
if(!violence){
  filename = "../../data/preprocessed/Table_construction.Rdata"
}else{
  filename = "../../data/preprocessed/Table_construction_arrests_hist_violence.Rdata"
}
